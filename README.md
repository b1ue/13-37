...
- **Compact scanner state** — one themed eye badge shows the number of enabled AirTag, Flipper, Skimmer, Evil Twin, and Flock detector tiles. Individual named badges remain hidden at zero and appear only after that detector records a hit, so enabling everything does not crowd the clock face.
...
- **Matrix background** — optional animated "digital rain" wallpaper with green or blue palettes. **Stock Data Rain** replaces random glyphs with configured symbols plus prices/percentage changes already cached by the Stocks screen. **System Data Rain** adds privacy-safe local telemetry—battery/charging state, uptime, free heap/PSRAM, WiFi state, active BLE scanner count, and dropped-advertisement count—with no SSID, MAC, or other identifier. Both feeds can run together, alternating by column. The **Rain Power** selector offers Eco, Balanced, and Smooth redraw budgets; none of these modes performs network or filesystem work from the animation timer.
...
- **Four-stage idle power policy** — Normal → Dim/Display Sleep → configurable **Light Standby** → long-idle **Deep Sleep**. Standby preserves RAM and resumes quickly from Power, BOOT, or the Ultra touch IRQ (when enabled and quiescent); its timer wake remains dark so the watch can continue into deep sleep without a panel flash. The existing deep-sleep path and battery savings are preserved. Both deadlines are independent, while alarms, timers/stopwatch, active radio/tool blockers, and USB-SD ownership retain their safety behavior. A normal daily alarm can wake either sleep state 15 seconds early.
- **Independent UI and rain palettes** — Matrix Green, Matrix Blue, and the squared amber/cyan **Retro Vector** instrumentation theme are semantic UI palettes. Matrix/Stock/System Rain keeps its own Green/Blue palette selection, so background data effects can coexist with any UI theme.
- **Verifiable firmware identity** — the boot splash and **Tools > About** show the semantic firmware version, feature-set label, and exact eight-character Git build revision. A `-dirty` suffix means the binary was compiled with uncommitted source changes. The serial monitor also reports the same identity and prints either deep-sleep entry or the active sleep blocker.
...
- **Remembered WiFi networks** — after a successful association, the watch keeps up to five networks in ESP32 NVS (never on the USB-visible SD card). Opening WiFi scans first and reconnects to the strongest visible saved network, so a deep-sleep reboot does not require retyping the password. Saved rows are labeled; long-press one to forget it. A failed saved-password attempt opens the keyboard and replaces the credential only after the retry succeeds. WiFi remains off during ordinary boot and sleep.

#### Stock bridge setup

1. Copy `tools/stock_bridge.example.json` to `tools/stock_bridge.json`. Quote-only use does not require a strategy program: leave the example preset alone and use **Refresh**, not **Run**, on the watch. Later, point the opaque preset's `cwd` and exact command at a private stock program; the bridge executes only commands in this allow-list.
2. Install and run it:

   ```bash
   python -m pip install -r tools/requirements-stock-bridge.txt
   # Generate once; store it in your shell/service environment, never in JSON or Git.
   export STOCK_BRIDGE_TOKEN="$(python -c 'import secrets; print(secrets.token_urlsafe(32))')"
   ...
   ```powershell
   Copy-Item tools\stock_bridge.example.json tools\stock_bridge.json
   py -m venv .stock-venv
   .\.stock-venv\Scripts\python.exe -m pip install -r tools\requirements-stock-bridge.txt
   $env:STOCK_BRIDGE_TOKEN = & .\.stock-venv\Scripts\python.exe -c "import secrets; print(secrets.token_urlsafe(32))"
   $env:STOCK_BRIDGE_TOKEN
   $env:ALPHAVANTAGE_API_KEY = "your-provider-key"
   .\.stock-venv\Scripts\python.exe tools\stock_bridge.py --config tools\stock_bridge.json
   ```

   The token-printing line is intentional: copy that value to the watch config,
   but do not save it in the repository. Find the PC's LAN address with
   `Get-NetIPAddress -AddressFamily IPv4`; before starting the bridge, change
   `host` in the copied JSON to that address and set `allow_remote` to `true`.

   The checked-in example binds only to `127.0.0.1`. To reach it from the watch,
   explicitly set `host` to the PC's LAN/VPN address (or `0.0.0.0`) and set
   `allow_remote` to `true`. Keep the port blocked from the public internet. The
   bridge refuses remote binding without that opt-in and accepts its 32+ character
   bearer token only from the configured environment variable.

   With the bridge running, verify it from a second PowerShell window before
   involving the watch:

   ```powershell
   $headers = @{ Authorization = "Bearer paste-the-same-token-here" }
   Invoke-RestMethod -Headers $headers "http://192.168.1.50:8737/api/watch/snapshot?symbols=AAPL,MSFT,SPY"
   ```
...
Plain HTTP is refused unless `allow_http=1`; use it only on a trusted isolated LAN or VPN. Prefer a TLS reverse proxy or VPN. For HTTPS, set `server_url=https://...`, leave `allow_http=0`, and copy the issuing CA certificate to `/Stocks/ca.pem` (or set `ca_file` to its SD path). The firmware validates that CA and never bypasses TLS verification. Treat `/Stocks/config.txt` as sensitive because it contains the bearer token.

The bridge first reads `snapshot_file` from its JSON config, allowing an existing private optimizer to publish quotes, alerts, and sparklines atomically. [`tools/watch_snapshot.example.json`](tools/watch_snapshot.example.json) documents the public boundary format. Missing quotes optionally fall back to server-side Alpha Vantage daily data, cached for five minutes by default.

Keep the strategy engine, model/parameter files, reports, and real preset mapping in a separate private directory or private repository. `tools/stock_bridge.json`, local environment files, credentials, and `stock-output/` are ignored by Git. The checked-in example uses only the opaque ID `private_job`; `expose_job_details` defaults to `false`, so the preset-list endpoint is closed and API responses do not return preset IDs, program output, or log tails. Obfuscation cannot make committed source private: anything pushed to this public fork should be assumed permanently visible.
...
- **NFC Read / Inspect** — scans NFC-A, NFC-B, NFC-F/FeliCa, NFC-V/ISO 15693, and ST25TB tags. The report identifies the RF technology and interface, UID, technology-specific activation metadata (including ATQA/SAK for NFC-A), NFC Forum tag type, NDEF version/state/capacity, every decoded record, and a raw-message preview. Wi-Fi keys are masked on screen. **Save** writes the complete report and full raw NDEF bytes to `/NFC/<timestamp>_<uid>.txt` only when explicitly tapped; saved raw records can contain credentials or other private tag data. The screen holds the **NFC power toggle** (DLDO1 rail) shared by both NFC screens — flip it on before reading or writing.
...
- **Rolling RX** — a deliberately **receive-only** 433.92/868.35 MHz pulse-timing analyzer for remotes you own or are authorized to test. It RSSI-slices strong nearby amplitude-modulated bursts, normalizes pulse durations into timing fingerprints, and compares consecutive captures as baseline, repeated/static, changed, different-family, or rolling-code candidate. A candidate requires multiple changing frames with a consistent timing family; it is evidence for further analysis, not decryption or proof of a particular algorithm. Captures and raw pulse durations are appended to `/Rolling/<session>.csv` when SD is available. The module has no transmit/replay API, and 315 MHz is omitted because this watch's front end measured below usable receive SNR there.
...
- **Packet Lab** — a receive-only 802.11 management-frame viewer for the internal 2.4 GHz WiFi radio. It can hop channels 1–13 or lock to one channel and apply bounded receive filters for subtype, minimum RSSI, or the selected frame's BSSID/MAC. It renders activity bars, lists recent frames, and shows source/destination/BSSID plus the first 80 captured bytes in hex. Each SD session writes a radiotap PCAP and a human-readable `.summary.txt` under `/Captures/`; RAM-only display still works without SD. There is deliberately no injection, replay, deauthentication, or transmit path.
- **Subnet-aware discovery and profiling** — Ping Sweep derives its CIDR from `WiFi.subnetMask()` instead of assuming `/24`; subnets larger than the watch can reasonably scan are explicitly labeled and sampled around the watch's address. mDNS, NBNS, DNS PTR, MAC and OUI evidence are retained separately, with bounded SSDP/UPnP and DNS-SD discovery. Resolved snapshots append to `/Network/host_profiles.csv` for later correlation.
- **Channel-quality analyzer** — the passive WiFi analyzer combines approximate AP counts, management/control/data frame activity, average/peak RSSI, and adjacent-channel pressure into a practical 1/6/11 recommendation rather than equating the fewest packets with the best channel.
- **Text Editor** — a compact on-watch editor using the LVGL touchscreen keyboard. Notes up to 4 KiB can be created, loaded, and overwritten in `/Notes/` on the microSD card. Filenames are confined to that directory and unsafe path characters are replaced. Read/write attempts are blocked with a clear error whenever USB Mass Storage owns the card.
...
- **Flipper** — passive BLE detector plus an authenticated Flipper Zero remote. Detection matches **two** signals so it catches custom firmwares too: (1) a complete- or shortened-local-name beginning with `Flipper ` (the stock-firmware advertised-name prefix), **or** (2) the Flipper BLE **service UUID `0x3082`** in the advertised 16-bit-UUID list. The second signal catches forks like **Momentum / Xtreme / Unleashed**, which may randomize the BLE name but still advertise the service UUID. Tap the tile to open **Flipper Remote**: its SCAN switch retains the passive detector, the target card shows the most recently seen device, and CONNECT opens the official encrypted BLE serial/RPC service. First pairing uses numeric comparison—the six-digit value must match on the physical Flipper before ACCEPT. Once connected, the watch exposes only an allow-listed D-pad (`UP/DOWN/LEFT/RIGHT/OK/BACK`) plus the official audiovisual **LOCATE** alert; it does not accept arbitrary protobuf, auto-launch RF/NFC/BadUSB apps, or auto-connect by name. The BLE controller remains staged through the shared manager, and leaving the screen disconnects the RPC session while the passive detector may keep scanning. When SD is available, unique detections are appended to `/Flipper/discovered.txt` with timestamp, MAC/address type, RSSI, advertised name, and optional GPS fix; hits are deduped for 5 minutes. The orange home badge still shows the running hit count.
...
- **Analyze** — three paired spectrum analyzers. Opens on the **WiFi** view, which hops promiscuous-mode capture across 2.4 GHz channels 1–13 and renders a utilisation chart. Swipe **left** to the **Bluetooth** view for BLE devices grouped into RSSI bands, then left again to the upgraded sub-GHz **Spectrum** view. Spectrum performs a receive-only 32-bin SX1262 RSSI sweep with a PSRAM-backed scrolling waterfall, live/peak-hold bars, exact current and peak frequencies, 315/433/868/915 MHz presets, Eco/Normal/Fast sweep rates, and optional CSV logging under `/Spectrum/`. The dedicated **Spectrum** Tools tile opens that view directly; swipe-right then returns to Tools, while entering through Analyze preserves the three-screen chain. This is an SDR-style sequential energy display—not wideband I/Q capture—and off-band sensitivity depends on the watch variant's RF matching. The analyzer snapshots a prior Pager/TPMS/APRS owner and restores it on exit or initialization failure.
...
- **Flock Detector** — passively flags configured surveillance-equipment signatures across the shared BLE/WiFi scanners and shows a count on the watch face. Each sighting is graded **Low** (advertised-name only), **Medium** (vendor OUI), or **High** (OUI and name agree); confidence means signal corroboration, not proof that a specific camera is present. Optional **Flock Alerts** add a five-second on-screen privacy banner and haptic pulse, while **Strong Only** limits those alerts to High matches. Local SD records include the match reason, RSSI, GPS fix quality, collision-safe per-hit files, and an offline-map-friendly `/Flock/sightings.csv`; nothing is uploaded. The standalone Flock toggle now starts only its low-duty passive BLE consumer; when Wardriver is active, its already-running WiFi scan feeds WiFi sightings into Flock too. This avoids cold-starting promiscuous WiFi from the Flock tile—the path implicated in UI freezes—and reduces standalone battery use. BLE registration and controller startup are deferred out of UI callbacks, and final controller shutdown remains asynchronous.

- **Concurrent BLE detector pipeline** — Bluetooth status, Wardriver, Analyze, AirTag, Flipper, Skimmer, and Flock can be enabled together through one controller scan. The Bluedroid callback only copies advertisements into a bounded queue; the main loop fans out at most two results per pass, preventing detector parsing or logging from blocking the Bluetooth system task. Cold controller and Bluedroid startup now advances one stage per loop after the tile has painted, matching the already-staged shutdown path. Ten consumer slots leave headroom for all current tools, Wardriver avoids re-running a detector already enabled by its own tile, and Flipper/Skimmer use fixed queues plus strict advertisement-length validation.
- **Concurrent WiFi detector pipeline** — Evil Twin and Wardriver share one promiscuous beacon scan. WiFi driver start/stop runs outside tile callbacks, received beacons enter a bounded queue, and detector processing occurs on the main loop. This keeps button/touch handling responsive while the radio changes mode and prevents WiFi callbacks from performing detector or SD work.
