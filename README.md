...
- **Matrix background** — optional animated "digital rain" wallpaper with green or blue palettes. **Stock Data Rain** replaces random glyphs with configured symbols plus prices/percentage changes already cached by the Stocks screen. It never fetches data itself, so the wallpaper does not keep WiFi or the CPU awake.
...
- **Display-off and deep-sleep policy** — set a **Dim Timer** and move **Dimmed** brightness to **OFF** for true panel sleep (`sleepDisplay()`), which is separate from merely drawing a black screen. The Sleep Timer then controls full ESP32-S3 deep sleep with four independent blockers: **Keep current screen awake**, **Keep WiFi awake**, **Keep radios awake**, and **Keep tools awake**. Turn all four off to let the idle timer stop WiFi, flush a running wardriver log, power down peripheral rails through LilyGoLib, and enter deep sleep. **1 Minute (Test)** and **5 Minutes** choices make it practical to verify that the board really sleeps before selecting a longer interval. Alarms, an active timer/stopwatch, and USB-SD host ownership always remain safety blockers. Wake with the power or boot button.
...
1. Copy `tools/stock_bridge.example.json` to `tools/stock_bridge.json`, then replace the opaque preset's `cwd` and exact command with the entry point for your private stock program. The bridge executes only commands in this allow-list.
...
- **Packet Lab** — a receive-only 802.11 management-frame viewer for the internal 2.4 GHz WiFi radio. It can hop channels 1–13 or lock to one channel, renders a live per-channel activity graph, lists recent beacon/probe/auth/deauth/action frames, and shows source/destination/BSSID plus the first 80 captured bytes in hex. Captures use a bounded queue so the WiFi callback never touches LVGL or the SD card. When SD is available, each session is written as a radiotap PCAP under `/Captures/` for desktop Wireshark; otherwise the display works in RAM-only mode. Starting is refused while WiFi, the shared beacon scanner, or USB SD owns the required hardware. There is deliberately no injection, replay, deauthentication, or transmit path.
